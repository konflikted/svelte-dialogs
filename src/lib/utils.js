...
import { group_outros, check_outros, transition_out } from "svelte/internal";
...
export const resolveConfigTransitions = (transitions) => {
  for (const key in transitions) {
    ...
    if (point && typeof point.transition === "string") {
      ...
      if (!transition)
        throw new Error(
          `${point.transition} not an existing svelte transition`
        );
...
/**
 * Utility function to apply a transition from the transitions configuration object
 * to an html target
 ...
 */
export const getOptionCompare = (multiple) =>
  multiple ? optionCompareMultiple : optionCompare;
